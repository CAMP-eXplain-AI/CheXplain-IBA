...
class Plotter:
  ...
  def plot_map(self, model, dataloader, label, methods=None, covid=False, saliency_layer=None, overlay=False, axes_a=None):
      ...
      for method, hmap in zip(methods, axes_a.flatten()[1:]):
          ...
          #TODO complete this two method
              ...
              saliency = self.iba(model, original, labels.squeeze(), bbox, model_loss_closure_with_target=self.softmax_crossentropy_loss_with_target)
          elif method == 'IB with reversed mask':
              saliency = self.iba(model, original, labels.squeeze(), bbox, reverse_lambda=True, model_loss_closure_with_target=self.softmax_crossentropy_loss_with_target)
  ...
  def iba(self, label, img, target, bbox, reverse_lambda=False, model_loss_closure_with_target=None):
    """
